<template>
	<div class="content">
		<div style="display: flex; justify-content: space-between">
			<div class="left">
				...
				<div>
					...
					<el-button style="margin-left: 30px" @click="deleteArt"
						>删除文章</el-button
					>
				</div>
				<div class="info">
		...
		<!-- <el-dialog title="评论" :visible.sync="dialogFormVisible">  sync已弃用 -->
		<el-dialog title="评论" v-model:visible.sync="dialogFormVisible">
			<textarea
				name=""
				id=""
				cols="30"
	...
	setup() {
		...
		const methods = {
			goHot(articleId) {
				...
				methods.getData(articleId);
			},
			nozan() {
				var articleId = parseInt(data.artId);
				apiDisLike(articleId)
					.then((res) => {
						...
						if (res.data.msg == "success") {
							...
							data.listData.articleLikeCount = res.data.data;
						} else {
							ElMessage({
								message: "没有权限",
								type: "fail",
					...
					.then((res) => {
						...
						if (res.data.data) {
							...
							router.push({
								name: "writeArticle",
								params: {
									articleTitle: data.listData.articleTitle,
									articleContent: data.listData.articleContent,
									id: 132132,
									articleCategoryName: data.listData.articleCategoryName,
									isEdit: true,
									articleId,
								},
							});
			...
			submitComment() {
				var articleId = parseInt(data.artId);
				//console.log(params)
				let params = `articleId=${articleId}&content=${data.textarea}`;
				// .post(url, params, {
				// 	headers: {
				// 		"content-type": "application/x-www-form-urlencoded",
				// 	},
				// })
				apiInsertArt(params).then(() => {
			...
			create() {
				data.artId = route.query.url;
				console.log(route.query.url);
				// data.artId = localStorage.getItem("article");
