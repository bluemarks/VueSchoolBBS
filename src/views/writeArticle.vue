	...
	<div>
		...
		<el-card style="height: 610px">
			<Quill-editor
				...
				ref="myQuillEditor"
				style="height: 500px"
				:options="editorOption.modules.toolbar.container"
			>
...
import { apiMode, apiPublish } from "@/request/article";
...
export default {
	...
	setup() {
		...
		const methods = {
			...
			getCookie() {
				...
				console.log("##cookiedObj:" + cookieObj);
				data.myCookie = cookieObj["Admin-Token"];
			},
			Return() {
			...
			mode() {
				var reg = /<\/?.+?\/?>/g;
				let articleTitle = data.input;
				let articleContent = data.content.replace(reg, "");
				var obj = {
					articleTitle: articleTitle,
					articleContent: articleContent,
					// console.log("###"+_self.ruleForm.resource),
					articleCategoryName: data.ruleForm.resource,
    ...
    onMounted(() => {
			methods.getCookie();
			console.log("页面传参11111", route);

			data.input = route.params.articleTitle;
			data.content = route.params.articleContent;
			data.ruleForm.resource = route.params.articleCategoryName;
			data.isEdit = route.params.isEdit;
